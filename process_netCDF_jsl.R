...
#summary stats
e<-aggregate(d$value~d$lat+d$lon, FUN=mean)
f<-aggregate(d$value~d$lat+d$lon, FUN=sd)
g<-aggregate(d$value~d$lat+d$lon, FUN=length)
h<-aggregate(d$value~d$lat+d$lon, FUN=var)
z<-data.frame(e,f$`d$value`,(f$`d$value`)^2,g$`d$value`)
names(z)<-c("lat","lon","mean","std","var","number")
